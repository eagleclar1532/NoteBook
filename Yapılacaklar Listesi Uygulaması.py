import tkinter
from tkinter import *
from datetime import datetime

root = Tk()
root.title("Yapılacaklar Listesi")
root.geometry("400x650+400+100")
root.resizable(False, False)

task_list = []

def addTask():
    task = task_entry.get()
    task_entry.delete(0, END)

    if task:
        date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # Tarih formatı
        task_with_date = f"{task} - {date}"

        with open("tasklist.txt", 'a') as taskfile:
            taskfile.write(f"\n{task_with_date}")
        
        task_list.append(task_with_date)
        listbox.insert(END, task_with_date)

def deleteTask():
    global task_list
    task = str(listbox.get(ANCHOR))
    if task in task_list:
        task_list.remove(task)
        with open("tasklist.txt", 'w') as taskfile:
            for task in task_list:
                taskfile.write(task + "\n")

        listbox.delete(ANCHOR)

def openTasFile():
    try:
        global task_list
        with open("tasklist.txt", "r") as taskfile:
            tasks = taskfile.readlines()

        for task in tasks:
            if task != '\n':
                task_list.append(task.strip())
                listbox.insert(END, task.strip())

    except:
        file = open('tasklist.txt', 'w')
        file.close()

# Simge=icon
Image_icon = PhotoImage(file="Image/task.png")
root.iconphoto(False, Image_icon)

# Üst Çubuk=Top Bar
TopImage = PhotoImage(file="Image/topbar.png")
Label(root, image=TopImage).pack()

dockImage = PhotoImage(file="Image/dock.png")
Label(root, image=dockImage, bg="#32405b").place(x=30, y=25)

noteImage = PhotoImage(file="Image/task.png")
Label(root, image=noteImage, bg="#32405b").place(x=340, y=25)

heading = Label(root, text="TÜM GÖREVLER", font="arial 20 bold", fg="white", bg="#32405b")
heading.place(x=130, y=20)

# Main
frame = Frame(root, width=400, height=50, bg="white")
frame.place(x=0, y=180)

task = StringVar()
task_entry = Entry(frame, width=18, font="arial 20", bd=0)
task_entry.place(x=10, y=7)
task_entry.focus()

button = Button(frame, text="EKLE", font="arial 20 bold", width=6, bg="#5a95ff", fg="#fff", bd=0, command=addTask)
button.place(x=300, y=0)

# Uyarı Mesajı (Türkçe Karakter Kullanılmaması için)
warning_label = Label(
    root,
    text="Lütfen Türkçe karakter kullanmayın! (Örnek: ç, ğ, ö, ş, ü, İ)",
    font=("arial", 10, "italic"),
    fg="red",
)
warning_label.place(x=30, y=120)

# Liste Kutusu=listbox
frame1 = Frame(root, bd=3, width=700, height=280, bg="#32405b")
frame1.pack(pady=(160, 0))

listbox = Listbox(frame1, font=('arial', 12), width=40, height=16, bg="#32405b", fg="white", cursor="hand2",
                  selectbackground="#5a95ff")
listbox.pack(side=LEFT, fill=BOTH, padx=2)
scrollbar = Scrollbar(frame1)
scrollbar.pack(side=RIGHT, fill=BOTH)

listbox.config(yscrollcommand=scrollbar.set)
scrollbar.config(command=listbox.yview)

openTasFile()

# Sil=DELETE
Delete_icon = PhotoImage(file="Image/delete.png")
Button(root, image=Delete_icon, bd=0, command=deleteTask).pack(side=BOTTOM, pady=13)

root.mainloop()
